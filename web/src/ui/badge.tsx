...
import { TagModel } from 'shared'
import { cn, tw } from './utils'

type BadgeVariant = 'default' | 'secondary' | 'destructive' | 'warning' | 'success' | 'outline' | TagModel.Color

const badgeVariants = cva(
  tw`focus:ring-ring inline-flex flex-none items-center rounded-full border px-3 py-1 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2`,
  {
    variants: {
      variant: {
        default: tw`bg-primary text-primary-foreground hover:bg-primary/80 border-transparent`,
        secondary: tw`bg-secondary text-secondary-foreground hover:bg-secondary/80 border-transparent`,
        destructive: tw`bg-destructive text-destructive-foreground hover:bg-destructive/80 border-transparent`,
        warning: tw`bg-warning text-warning-foreground hover:bg-warning/80 border-transparent`,
        success: tw`bg-success text-success-foreground hover:bg-success/80 border-transparent`,
